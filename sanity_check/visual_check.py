...
def main():
    ...
    lon = 1#10
    ts = cfact[variable].sel(lon=lon, lat=lat, method='nearest')
    ts_f = fact[variable].sel(lon=lon, lat=lat, method='nearest')
    ts = ts[timestep: ]  # get only timesteps of around last 3 year
    ts_f = ts_f[timestep: ]

    fig, ax = plt.subplots()
    ts_f.plot(ax=ax, color="Red")
    ts.plot(ax=ax)
    ...
    value_min = np.min(cfact[0, :, :]) - 5  # TODO find most extreme values in cfact or fact
    value_max = np.max(cfact[0, :, :]) + 5


    fig, (ax1, ax2) = plt.subplots(figsize = (18, 3), ncols = 2)
    #norm = mcolors.DivergingNorm(vmin=-0.1, vmax=np.max(fact[timestep,  :, :]), vcenter=0.0) # aligne around 0
    #f = ax1.imshow(fact[0,  :, :], cmap="RdYlBu", vmin=value_min, vmax=value_max) #, norm=norm) #, vmin=value_min, vmax=value_max)
    f = fact[0,:,:].plot(cmap="RdYlBu", vmin=value_min, vmax=value_max)
    fig.colorbar(f, ax = ax1)
    ax1.set_title(f"fact {variable}, \ntimestep= {date_of_timestep}")

    #norm = mcolors.DivergingNorm(vmin=-0.1, vmax=np.max(cfact[timestep,  :, :]), vcenter=0.0) # aligne around 0
    #c = ax2.imshow(cfact[0,  :, :], cmap="RdYlBu", vmin=value_min, vmax=value_max) #, norm=norm)#, vmin= value_min, vmax=value_max) 
    c = cfact[0,:,:].plot(cmap="RdYlBu", vmin=value_min, vmax=value_max)
    fig.colorbar(c, ax = ax2)
    ax2.set_title(f"cfact {variable}, \ntimestep= {date_of_timestep}")
    ...
    value_min = np.min(cfact_mean[ :, :]) -5 
    value_max = np.max(cfact_mean[ :, :]) + 5
